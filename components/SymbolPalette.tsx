        ...
        <div className="grid grid-cols-6 gap-2 sm:grid-cols-8 md:grid-cols-10">
          {SYMBOL_CATEGORIES[activeTab].symbols.map((symbol, index) => (
            <button
              ...
              className="p-2 text-center border border-gray-200 rounded hover:bg-gray-50 hover:border-blue-300 transition-colors"
              title={symbol.latex}
            >
              <span className="text-lg">{symbol.label || symbol.latex}</span>
            </button>
          ))}
        </div>
      </div>
